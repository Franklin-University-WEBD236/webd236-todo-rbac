...
class GroupModel extends Model {
  ...
  public static function findAll() {
    $st = self::$db -> prepare("SELECT * FROM groups_ ORDER BY name");
    $st -> execute();
    return self::fromRows($st -> fetchAll(PDO::FETCH_ASSOC));
  }

  public static function findById($id) {
    $st = self::$db -> prepare("SELECT * FROM groups_ WHERE id = :id");
    $st -> bindParam(':id', $id);
    $st -> execute();
  ...
  public static function findByName($name) {
    $st = self::$db -> prepare("SELECT * FROM groups_ WHERE name = :name");
  ...
  public function insert() {
    $this -> validate(true);
    $this -> clean();
    $statement = self::$db -> prepare("INSERT INTO groups_ (name) VALUES (:name)");
  ...
  public function update() {
    $this -> validate(true);
    $this -> clean();
    $statement = self::$db -> prepare("UPDATE groups_ SET name = :name WHERE id = :id");
    $statement -> bindParam(':name', $this -> name);
  ...
  private static function deleteById($id) {
    ...
    print_r($id);
    $statement = self::$db -> prepare("DELETE FROM groups_ WHERE id = :id");
  ...
  public static function findByUserId($userId) {
    $statement = self::$db -> prepare("SELECT * FROM groups_ where groups_.id in (SELECT groups_.id from groups_, usergroups WHERE groups_.id = usergroups.groupId AND usergroups.userId = :userId) order by name");
  ...
  public static function findByNotUserId($userId) {
    $statement = self::$db -> prepare("SELECT * FROM groups_ where groups_.id not in (SELECT groups_.id from groups_, usergroups WHERE groups_.id = usergroups.groupId AND usergroups.userId = :userId) order by name");
    $statement -> bindParam(':userId', $userId);
    $statement -> execute();
    return self::fromRows($statement->fetchAll(PDO::FETCH_ASSOC));
  }
